...
main(int argc, char *argv[])
{
    ...
    char *mirror_word = "mirror";
    char *mirror_msg = "Port Mirroring functionality is not available in this OVS version";

    char *ingress_rate = "ingress_policing_rate";
    char *ingress_burst = "ingress_policing_burst";
    char *interface_msg = "Interface Rate Limiting is not availabe in this OVS version";

    char *vtep_word = "vtep";
    char *vtep_msg  = "VTEP functionality is not available in this OVS version";

    for(int ab = 0 ; ab < argc ; ab ++) {  	
    	if(match(argv[ab] ,mirror_word) == 1) {
		printf("%s\n",mirror_msg);
		return 0;
   	}

	if(match(argv[ab], ingress_rate) == 1 || match(argv[ab], ingress_burst) == 1) {
		printf("%s\n", interface_msg);
		return 0;	
	}

	if(match(argv[ab], vtep_word) == 1) {
		printf("%s\n", vtep_msg);
		return 0;
	}		
    }
